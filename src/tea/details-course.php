...
$Relation_query = "SELECT * FROM `course` WHERE `courseid` = ('$courseid') AND `teaid` = ('$teaid')";
$RelationRS = mysqli_query($dbcon, $Relation_query) or die(mysqli_error($dbcon));
$RelationConfirm = mysqli_num_rows($RelationRS); //课程所属与当前教师不符，越权处理
if(!$RelationConfirm){
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "Courseform")) {
	...
	if($_SESSION['MM_tea_CourseModAllow'] == 0){
		header("Location: ../errors/unauthorized.php");
		exit(1);
	}
...
if ((isset($_GET['courseid'])) && ($_GET['courseid'] != "") && ($_GET['delstuid']) && ($_GET['delstuid'] != "")) {
	if($_SESSION['MM_tea_StuDelAllow'] == 0){
		header("Location: ../errors/unauthorized.php");
		exit(1);
	}
    // 转义
...
if ((isset($_GET['courseid'])) && ($_GET['courseid'] != "") && ($_GET['finstuid']) && ($_GET['finstuid'] != "")) {
	if($_SESSION['MM_tea_StuFinAllow'] == 0){
		header("Location: ../errors/unauthorized.php");
		exit(1);
	}
    // 转义
						...
						<table class="table table-striped table-hover table-sm">
							...
							<tbody>
								<tr>
									...
									<td><button <?php if($_SESSION['MM_tea_CourseModAllow']==0)echo "hidden"; ?> type="submit" name="submit" class="btn btn-primary">修改</button></td>
								</tr>
							</tbody>
						</table>
						...
						<table class="table table-striped table-hover table-sm">
							...
							<tbody  id="StuTable">
								...
								<tr>
									...
									<td>
									<a class="btn btn-danger active btn-sm <?php if($_SESSION['MM_tea_StuDelAllow']==0)echo "disabled"; ?>" onclick="return confirm('确定要删除吗？')" href="details-course.php?courseid=<?php echo $row_Recordset1['courseid']; ?>&delstuid=<?php echo $row_Recordset2['stuid']; ?>">删除</a>
									<a class="btn active btn-sm <?php if($row_Recordset2['fin']=='1' || $_SESSION['MM_tea_StuFinAllow']==0){echo "disabled btn-secondary";}else{echo "btn-primary";} ?>" href="details-course.php?courseid=<?php echo $row_Recordset1['courseid']; ?>&finstuid=<?php echo $row_Recordset2['stuid']; ?>"><?php if($row_Recordset2['fin']=='1'){echo "课程已通过";}else{echo "通过课程";} ?></a>
									</td>
								</tr>
